...
class GamificationService {
  ...
  /// dev session equip any royal. This field itself is never written to
  /// storage; while dev mode is on it is re-applied at startup from DevMode's
  /// persisted overlay (so the previewed royal survives a restart), and cleared
  /// the moment dev mode is turned off — landing back on the user's real,
  /// earned avatar.
  ...
  Future<GamiProfile> loadProfile() async {
    ...
    //
    // BEFORE the dev-mode overlay below, and that order is load-bearing on this
    // branch: banking reads what is actually PERSISTED. Applied after the
    // override it would read the preview instead, and a cosmetic a developer
    // was only trying on would be written into real, permanent entitlement —
    // the exact leak the overlay exists to prevent.
